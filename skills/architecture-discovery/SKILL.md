...
description: >-
  ...
  an unfamiliar project, bootstrap docs for a project that has none, or find
...
3. **Identify candidate decisions.** Surface architectural choices visible in
   ...
   choice; it could be a leftover experiment or an accident). Mark these as
   **candidate ADRs** (see `references/terminology.md`) requiring
   confirmation, not settled decisions.
4. **Assess doc gaps.** Compare what exists (`README`, `ARCHITECTURE.md`,
   `docs/`, ADRs, diagrams) against what the discovered architecture
   warrants — coverage only. Whether the existing documentation is
   internally consistent with itself is `architecture-librarian`'s check,
   not this one.
